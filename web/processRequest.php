...
// -------
//
function qruqsp_fielddaylog_web_processRequest(&$ciniki, $settings, $tnid, $args) {

    //
    // Check to make sure the module is enabled
    //
    if( !isset($ciniki['tenant']['modules']['qruqsp.fielddaylog']) ) {
        return array('stat'=>'404', 'err'=>array('code'=>'qruqsp.fielddaylog.5', 'msg'=>"I'm sorry, the page you requested does not exist."));
    }

    if( $args['page_title'] == '' ) {
        $args['page_title'] = 'Field Day';
    }
    $page = array(
    ...
    if( count($page['breadcrumbs']) == 0 ) {
        if( isset($settings['page-fielddaylog-name']) && $settings['page-fielddaylog-name'] != '' ) {
            $page['breadcrumbs'][] = array('name'=>$settings['page-fielddaylog-name'], 'url'=>$args['base_url']);
        } else {
