...
// -------
//
function qruqsp_fielddaylog_qsoDelete(&$ciniki) {
    //
    ...
    $rc = qruqsp_fielddaylog_checkAccess($ciniki, $args['tnid'], 'ciniki.fielddaylog.qsoDelete');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( !isset($rc['qso']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.fielddaylog.4', 'msg'=>'QSO does not exist.'));
    }
    $qso = $rc['qso'];
    ...
    // Remove the qso
    //
    $rc = ciniki_core_objectDelete($ciniki, $args['tnid'], 'qruqsp.fielddaylog.qso',
